...
typedef struct RMF_AudioCapture_Settings {
    ...
    unsigned int    delayCompensation_ms;           //!< Delay compensation in milli seconds. This parameter is used to maintain AV sync when using latency-prone audio outputs like Bluetooth. It's the number of ms to delay video by in order to stay in sync with Bluetooth audio
...
typedef struct RMF_AudioCapture_Status {
    ...
    int             muted;                  //!< Indicates whether capture is muted
    int             paused;                 //!< Indicates whether capture is paused
 ...
 * @param [out] status  - Status returned by the underlying implementation. 
 *                        The life-cycle of status will be managed by the caller.
 *
 * @return rmf_Error
 * @retval RMF_ERROR General error
 * @retval RMF_INVALID_PARM Invalid parameter
 ...
 *
 * @param [out] settings   - Default values for audio capture settings. 
 *                           The life-cycle of settings will be managed by the caller.
 *
 * @return rmf_Error
 * @retval RMF_ERROR General error
 * @retval RMF_INVALID_PARM Invalid parameter
 ...
 *
 * @param [in] handle    - Handle of the audio capture interface.
 * @param [out] settings - current values of audio capture Settings. 
 *                         The life-cycle of settings will be managed by the caller.
 *
 * @return rmf_Error
 * @retval RMF_ERROR General error
 ...
 *
 * @param [in] handle    - Handle of the audio capture interface.
 * @param [in] settings  - Capture settings to use. 
 *                         The life-cycle of settings will be managed by the caller.
 *
 * @return rmf_Error
 * @retval RMF_ERROR General error
